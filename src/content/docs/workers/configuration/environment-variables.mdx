...
Environment variables are a type of binding that allow you to attach text strings or JSON values to your Worker. They are passed in via the [`env` parameter](/workers/runtime-apis/handlers/fetch/#parameters) in your Worker's [`fetch` event handler](/workers/runtime-apis/handlers/fetch/).
